...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (!authLoading && isAuthenticated) {
      setTimeout(() => router.push("/"), 100); // Slight delay to ensure state updates
    }
    ...
    try {
      ...
      if (response.ok && data.success) {
        const userData = {
          ...
          role: data.user.role || loginType,
          loginType: data.user.role || loginType,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-purple-100 to-slate-200 flex items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 items-start relative z-10">
        {/* Left Side - Login Form or User Credentials */}
          ...
          <Card className="border-0 shadow-2xl backdrop-blur-xl bg-white/95 overflow-hidden">
            <CardHeader className="pb-8">
              ...
              <CardTitle className="text-3xl font-bold text-gray-900 text-center">
                {isAuthenticated ? `Hello, ${user?.name || "User"}!` : "Welcome Back"}
              </CardTitle>
              <CardDescription className="text-gray-600 text-center text-lg">
                {isAuthenticated ? "Your account details" : "Sign in to access your academic dashboard"}
              </CardDescription>
            </CardHeader>
              ...
              {isAuthenticated && user ? (
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="p-3 bg-gray-50 rounded-lg text-gray-800 capitalize">{user.role}</div>
                  </div>
                  {user.department && (
                    <div className="space-y-2">
                      <Label className="text-base font-semibold text-gray-700">Department</Label>
                      <div className="p-3 bg-gray-50 rounded-lg text-gray-800">{user.department}</div>
                    </div>
                  )}
                  {user.facultyId && (
                    <div className="space-y-2">
                      <Label className="text-base font-semibold text-gray-700">Faculty ID</Label>
                ...
                <>
                  ...
                  <div className="text-center pt-4">
                    <Link
                      href="/"
                      className="text-sm text-gray-600 hover:text-purple-600 font-medium transition-colors duration-200 flex items-center justify-center space-x-2"
                    >
                      <span>←</span>
        ...
        {/* Right Side - Conditional Rendering: Demo Credentials or User Details */}
        <div className="w-full max-w-md mx-auto">
          <Card className="border-0 shadow-2xl backdrop-blur-xl bg-white/95 overflow-hidden">
            <CardHeader className="pb-4">
              <CardTitle className="text-2xl font-bold text-gray-900 text-center">
                {isAuthenticated ? "Your Profile Summary" : "Demo Credentials"}
              </CardTitle>
              <CardDescription className="text-gray-600 text-center text-base">
                {isAuthenticated ? "Overview of your account" : "Use these to test the platform"}
              </CardDescription>
            </CardHeader>
